...
class IlaiGuide {
  static IlaiTopic topic(String question) {
    ...
    if (RegExp(r'connect|offline|wifi|wi-fi|firebase|இணை|சாதனம்').hasMatch(q))
      return IlaiTopic.connection;
    if (RegExp(r'why|reason|evidence|ஏன்|எதனால்|காரண').hasMatch(q))
      return IlaiTopic.reason;
    if (RegExp(
            r'what.*do|action|solution|water|help.*plant|செய்|தீர்வு|தண்ணீர்|நீர்')
        .hasMatch(q)) return IlaiTopic.action;
    if (RegExp(r'problem|wrong|plant|health|condition|செடி|நிலை|பிரச்ச')
        .hasMatch(q)) return IlaiTopic.summary;
    return IlaiTopic.help;
  }

  static String reply(IlaiTopic topic, IlaiSnapshot data,
      {required bool tamil}) {
    String t(String en, String ta) => tamil ? ta : en;
    if (topic == IlaiTopic.help) {
      return t(
          'I’m Ilai, PhytoSense’s plant guide. I can explain the current plant finding, the next step, and the reason. I’m a guided assistant, not a general AI chatbot. Try one of the questions below.',
    ...
    if (topic == IlaiTopic.connection) {
      return t(
          'Open Settings to connect your ESP32. Nearby, use the device’s Wi-Fi. Away from it, use your configured remote connection. Wait for a fresh reading. Simulation is only for practice and is kept separate.',
    ...
    if (!data.available) {
      return data.simulation
          ? t('No practice reading is available. Choose a practice condition in Settings, then ask again.',
              'பயிற்சி அளவீடு கிடைக்கவில்லை. அமைப்புகளில் பயிற்சி நிலையைத் தேர்வு செய்து மீண்டும் கேளுங்கள்.')
    ...
    final missing = t(
        'The device has not supplied this detail. Open Plant care for the available findings.',
