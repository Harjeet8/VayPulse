...
Widget dial(double? score, {bool reduced = true}) => MaterialApp(
      home: MediaQuery(
        ...
        child: Scaffold(
          ...
          body: Center(
              child: HealthArc(
                  score: score, compact: true, statusColor: Colors.red)),
        ),
      ),
    );

void main() {
  testWidgets('full colour scale stays visible at low and high scores',
      (tester) async {
    for (final score in [0.0, 33.0, 100.0]) {
      ...
      await tester.pumpWidget(MaterialApp(
        ...
        child: Scaffold(
          ...
          key: boundary,
          child:
              HealthArc(score: score, compact: true, statusColor: Colors.red),
        ))),
      )));
      await tester.pump();
  ...
  testWidgets('losing a reading immediately removes coloured live marker',
      (tester) async {
    ...
    expect(
        find.byKey(const ValueKey('health-dial-unavailable')), findsOneWidget);
  ...
  testWidgets(
      'invalid readings stay unavailable instead of becoming healthy or zero',
      (tester) async {
