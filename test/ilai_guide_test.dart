...
void main() {
  ...
  test('guide repeats supplied advice without deriving a new diagnosis', () {
    const data = IlaiSnapshot(
        available: true,
        simulation: false,
        condition: 'Needs checking',
        problem: 'Verify soil probe placement',
        action: 'Check the sensor before watering',
        reason: 'Probe outside reference range');
    ...
    expect(IlaiGuide.reply(IlaiTopic.reason, data, tamil: false),
        contains(data.reason));
  });
  test('unavailable hardware never repeats stale supplied advice', () {
    const stale =
        IlaiSnapshot(available: false, simulation: false, action: 'Water now');
    for (final topic in [
      IlaiTopic.summary,
      IlaiTopic.action,
      IlaiTopic.reason
    ]) {
      expect(IlaiGuide.reply(topic, stale, tamil: false),
          isNot(contains('Water now')));
    }
  });
  test('simulation and absent device details are explicit in both languages',
    ...
    expect(IlaiGuide.reply(IlaiTopic.action, data, tamil: false),
        contains('Practice data only'));
    expect(IlaiGuide.reply(IlaiTopic.reason, data, tamil: true),
        contains('பயிற்சிக்கான தரவு மட்டும்'));
    expect(IlaiGuide.reply(IlaiTopic.action, data, tamil: false),
        contains('has not supplied'));
  });
  test(
      'English and Tamil questions choose supported topics; unknown asks get help',
      () {
  ...
  test('voice names stay stable across ordering and available-voice changes',
      () async {
    ...
    final reordered =
        await VoiceNames.forVoices([b, a], 'en', tamilLabels: false);
    expect(reordered, first);
    expect(first[a.name], isNot(first[b.name]));
    expect((await VoiceNames.forVoices([b], 'en', tamilLabels: false))[b.name],
        first[b.name]);
